...
export default function RefundPage() {
  ...
  async function loadRefundHint(campaignKey: PublicKey) {
    ...
    try {
      ...
      const campaign = (await program.account.campaign.fetch(campaignKey)) as CampaignAccount;
      ...
      if (request !== null && Number.isInteger(request) && request >= 0) {
        ...
        try {
          const wr = (await program.account.withdrawRequest.fetch(withdrawKey)) as WithdrawAccount;
  ...
  async function handleRefund() {
    ...
    if (requestIndex && (!Number.isInteger(indexValue) || indexValue < 0)) {
